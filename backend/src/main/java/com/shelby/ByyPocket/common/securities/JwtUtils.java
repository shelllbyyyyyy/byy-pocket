...
import static com.shelby.ByyPocket.common.Constants.*;
...
public class JwtUtils {
    ...
    @Value("${app.security.verificationJwtSecret}")
    private String verificationJwtSecret;
    ...
    public String generateTokenLink(String data) {
        return Jwts.builder()
                .setSubject(data)
                .setIssuedAt(new Date())
                .setExpiration(new Date((new Date()).getTime() + 36000))
                .signWith(SignatureAlgorithm.HS512, verificationJwtSecret)
                .compact();
    }
    ...
    public String getEmailFromJwtToken(String token) {
        return Jwts.parser().setSigningKey(verificationJwtSecret).parseClaimsJws(token).getBody().getSubject();
    }

    public boolean validateUser(String authToken) {
        try {
            Jwts.parser().setSigningKey(verificationJwtSecret).parseClaimsJws(authToken);
            return true;
